...
export const getArticles = sort => {
  return async (dispatch, getState) => {
    try {
      ...
      // dispatch(articles.successGlobal('Very Good'))
    } catch (error) {
      dispatch(articles.errorGlobal('error loading more articles'))
    }
  }
}
...
export const getPaginateArticles = (page = 1, limit = 8) => {
  return async dispatch => {
    try {
      const request = await axios.post(
...
export const changeStatusArticle = (status, _id) => {
  return async (dispatch, getState) => {
    try {
      ...
      const state = getState().articles.adminArticles.docs // / previous state
      const position = state.findIndex(art => art._id === _id) // / find the position
      state[position] = art

      dispatch(articles.updateArticleStatus(state))
      dispatch(articles.successGlobal('Good!'))
    } catch (error) {
      dispatch(articles.errorGlobal(error.response.data.message))
    }
  }
...
export const addCategory = values => {
  return async (dispatch, getState) => {
    try {
      ...
      const newState = [...getState().articles.categories, category.data]
